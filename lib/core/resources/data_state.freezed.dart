...
class _$DataStateTearOff {
  ...
  Failure<T> failure<T>(String message, [Exception? exception]) {
    return Failure<T>(
      message,
      exception,
    );
...
class _$Failure<T> implements Failure<T> {
  const _$Failure(this.message, [this.exception]);
  ...
  @override
  String toString() {
    return 'DataState<$T>.failure(message: $message, exception: $exception)';
  }
...
abstract class Failure<T> implements DataState<T> {
  const factory Failure(String message, [Exception? exception]) = _$Failure<T>;
